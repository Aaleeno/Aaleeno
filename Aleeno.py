...
Insights
 main 
DECOMPILED-XD/Aaleeno.py

AALEENO-XD Add files via upload
 1 contributor
1459 lines (1363 sloc)  94.9 KB
#!/usr/bin/python3
#-*-coding:utf-8-*-
# Update V1.6
...
def banner():
           ...
           _      ______ ______ _   _  ____  
     /\   | |    |  ____|  ____| \ | |/ __ \ 
    /  \  | |    | |__  | |__  |  \| | |  | |
   / /\ \ | |    |  __| |  __| | . ` | |  | |
  / ____ \| |____| |____| |____| |\  | |__| |
 /_/    \_\______|______|______|_| \_|\____/ 
...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO ALEENA TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sALEENA-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sALEENA-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("ALEENA-CP")
    except:pass
    try:os.mkdir("ALEENA-OK")
    except:pass
...
def genkey():
	try:
		...
		print("\033[0;93m   TOOL PRICE : %s350 "%(H))
		print("\033[0;93m   VALIDITY : %s1 week"%(H))
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF ALEENA%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
		print("\033[0;93m               SEND THIS KEY TO OWNER \n\n YOUR KEY : \033[0;92m%s%s"%(rb,basesplit))
		print("\033[0;93m    TOOL PRICE : %sRs350"%(H))
		print("\033[0;93m    VALIDITY : %s1 Week"%(H))
		subprocess.check_output(["am", "start", "https://wa.me/+17372766883"])
		exit()
...
About
Loading complete"
 https://github.com/SHANKAR-XD/DECOMPILED-XD/blob/main/FUCK-KRISHNA.py#:~:text=Skip%20to%20content,Loading%20complete
